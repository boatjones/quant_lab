...
def build_iso(date_str, default_time, tzname):
    ...
    z = ZoneInfo(tzname)
    return datetime(dt.year, dt.month, dt.day, hh, mm, ss, tzinfo=z).isoformat()
...
def main():
    ...
    skip = set(DEFAULT_SKIP_SECURITIES)

    trades = df.loc[df["Action"].isin(list(BUY_ACTIONS|SELL_ACTIONS)) & (~df["Security"].isin(skip))].copy()
    ...
    trades["date_iso"] = [build_iso(d, args.default_buy_time if a in BUY_ACTIONS else args.default_sell_time, args.tz) for d,a in zip(trades["Date"], trades["Action"])]

    out = pd.DataFrame({
        "date": trades["date_iso"],
    ...
    if args.emit_cashflows:
        reinv = df.loc[df["Action"].isin(list(REINV_ACTIONS)) & (~df["Security"].isin(skip))].copy()
        if os.path.exists(args.symbols_map):
            sm = pd.read_csv(args.symbols_map)
            ...
            reinv["symbol"] = ""
        reinv["date_iso"] = [build_iso(d, args.default_buy_time, args.tz) for d in reinv["Date"]]
        amt_col = None
        for c in reinv.columns:
            if c.replace("\xa0"," ") == "Amount Invested":
                ...
                break
        if amt_col is None:
            amount = pd.to_numeric(reinv["Quote/Price"].str.replace(",",""), errors="coerce") * pd.to_numeric(reinv["Shares"].str.replace(",",""), errors="coerce")
        else:
