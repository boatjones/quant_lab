...
# Cash dividend-like actions we will include as dividend cashflows
...
def normalize_cols(df):
    # Create easy handles to columns that may contain NBSPs
    cols_norm = {c.replace("\xa0"," ").replace("\u00a0"," "): c for c in df.columns}
    return cols_norm
...
def main():
    ...
    # focus: equity trades only; skip money-market/cash lines
    ...
    if args.emit_cashflows:
        ...
        # Amount for reinvests: prefer "Amount Invested" (NBSP-safe), else price*shares
        amt_inv_col = None
        ...
        # --- Cash dividends block ---
        # Heuristic: any Action in CASH_DIV_ACTIONS, AND not a money-market-only "Security"
        ...
        if os.path.exists(args.symbols_map):
            ...
            cashdiv["symbol"] = ""
        cashdiv["date"] = [build_iso(d, "12:00:00", args.tz) for d in cashdiv["Date"]]  # neutral midday default

        # Amount for cash dividends: prefer "Cash" column; if absent, fall back to any Amount/Net field
        cash_col = "Cash" if "Cash" in cashdiv.columns else None
        if cash_col is None:
            # try a flexible scan for any 'Amount' column
            for c in cashdiv.columns:
                if c.lower() in {"amount","net","net amount"} or c.replace("\xa0"," ").replace("\u00a0"," ").lower() in {"amount","net","net amount"}:
                    cash_col = c
                    break
        if cash_col:
            cash_amount = pd.to_numeric(cashdiv[cash_col].str.replace(",",""), errors="coerce")
        else:
            # last resort: price*shares (rare for plain Div)
            cash_amount = pd.to_numeric(cashdiv["Quote/Price"].str.replace(",",""), errors="coerce") * \
        ...
        # Remove zero/negative dividends if any spurious entries
